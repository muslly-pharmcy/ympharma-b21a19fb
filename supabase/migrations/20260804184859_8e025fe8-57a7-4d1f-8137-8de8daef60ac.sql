...
SELECT
  ...
  p.category_id,
  p.updated_at,
  COALESCE((SELECT sum(b.qty_on_hand) FROM public.inv_stock_batches b
            WHERE b.product_id = p.id AND b.organization_id = p.organization_id), 0::numeric) AS stock_balance
